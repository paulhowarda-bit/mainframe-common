...
bundle's (:mod:`mainframe_artifacts.bundle`), because they earned their keep there:
...
* **Replay introduces no new branch.** A consumer that accepts a parse bundle skips
  ...
  :class:`~cobol_parser.model.Program`; everything downstream is the same code, byte for
...
The contract is versioned against :mod:`cobol_parser.model`: any change to a field set
