...
``--koopa-jar`` or ``COBOL_PARSER_KOOPA_JAR`` at a release jar.
"""
...
JAR_ENV = "COBOL_PARSER_KOOPA_JAR"
...
def run_koopa(lines: List[CodeLine], jar: Optional[str] = None) -> ET.Element:
    ...
    java = require_java()
    with tempfile.TemporaryDirectory(prefix="cobol-parser-koopa-") as td:
...
def diff_producers(program, root: ET.Element, lines: List[CodeLine]) -> dict:
    ...
    return {
        "format": "cobol-parser-producer-diff",
        "version": 1,
        "producers": {"native": "cobol-parser-python", "external": "koopa"},
        "totals": {
