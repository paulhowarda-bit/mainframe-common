...
from mainframe_artifacts.cliargs import (add_logging_args, add_retrieval_args,
                                       jobs as _jobs)
...
# Explicit name, NOT __name__: this module is also run as `python -m cobol_parser`,
# where __name__ would put the logger outside configure_logging's reach.
_log = logging.getLogger("cobol_parser.cli")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--diff-producers", action="store_true",
                   help="also run the external Koopa parser (Java, BSD) over the same "
                        ...
                        "Koopa release jar (--koopa-jar or COBOL_PARSER_KOOPA_JAR).")
    p.add_argument("--koopa-jar", metavar="JAR",
                   help="path to a Koopa release jar "
                        ...
                        "bundled. Defaults to $COBOL_PARSER_KOOPA_JAR.")
    add_logging_args(p)
    return p
