...
command, so pip resolves `cobol-parser`'s dependency on core from the explicit reference
rather than looking for it on an index:

```bash
pip install "mainframe-artifacts @ git+https://github.com/paulhowarda-bit/mainframe-common#subdirectory=mainframe-artifacts" "cobol-parser @ git+https://github.com/paulhowarda-bit/mainframe-common#subdirectory=cobol-parser"
```

## Use

```bash
cobol-parser prog.cbl -o prog.parse.json    # source -> parse bundle (serialized Program)
```
...
--from-parse prog.parse.json` models from it without re-parsing. See `cobol-parser/README.md`
...
The ratchet hashes the parse bundle a real `cobol-parser <example> --no-fetch` run writes
